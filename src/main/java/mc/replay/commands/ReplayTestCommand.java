...
public class ReplayTestCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args[0].equalsIgnoreCase("start")) {
            ...
            recordables = MCReplay.getInstance().getReplayStorage()
                    .getTypeRecordables(EntityRecordable.class, target.getUniqueId());

            MCReplay.getInstance().getSessions().put(player, new ReplaySession(recordables, List.of(player)));
        ...
        if (args[0].equalsIgnoreCase("save")) {
            ...
            MCReplay.getInstance().createReplay(player);
