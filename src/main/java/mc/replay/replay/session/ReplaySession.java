...
public final class ReplaySession {
    ...
    public ReplaySession(NavigableMap<Long, List<Recordable>> recordables, List<Player> targets) {
        ...
        this.task = Bukkit.getScheduler().runTaskTimer(MCReplay.getInstance(), () -> {
        ...
        this.informTask = Bukkit.getScheduler().runTaskTimer(MCReplay.getInstance(), () -> {
    ...
    public void jumpForwards(int seconds) {
        ...
        this.entities.removeIf((entity) -> {
            NavigableMap<Long, List<Recordable>> recordables = MCReplay.getInstance().getReplayStorage().getTypeRecordables(records, EntityRecordable.class, entity.getOriginalEntityId());
