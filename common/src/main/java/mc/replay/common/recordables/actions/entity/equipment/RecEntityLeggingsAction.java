...
import mc.replay.common.recordables.types.entity.equipment.RecEntityLeggings;
...
public final class RecEntityLeggingsAction implements EntityRecordableAction<RecEntityLeggings> {

    @Override
    public @NotNull List<@NotNull ClientboundPacket> createPackets(@NotNull RecEntityLeggings recordable, @UnknownNullability IEntityProvider provider) {
        ...
        if (data.entity() instanceof RLivingEntity livingEntityWrapper) {
            livingEntityWrapper.setEquipment(EquipmentSlot.LEGS, recordable.item());
        }
        ...
        return List.of(
                new ClientboundEntityEquipmentPacket(
                        data.entityId(),
                        Map.of(
                                (byte) EquipmentSlot.LEGS.ordinal(),
                                recordable.item()
                        )
                )
        );
