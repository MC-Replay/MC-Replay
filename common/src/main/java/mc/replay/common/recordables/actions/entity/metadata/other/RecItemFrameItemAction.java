...
public record RecItemFrameItemAction() implements EntityRecordableAction<RecItemFrameItem> {

    @Override
    public @NotNull List<@NotNull ClientboundPacket> createPackets(@NotNull RecItemFrameItem recordable, @UnknownNullability IEntityProvider provider) {
        ...
        if (entityMetadata instanceof ItemFrameMetadata itemFrameMetadata) {
            Metadata metadata = itemFrameMetadata.getMetadata();
