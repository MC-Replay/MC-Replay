...
public record RecEntityOffHand(EntityId entityId, ItemWrapper item) implements EntityStateRecordable {

    public RecEntityOffHand(EntityId entityId, ItemStack itemStack) {
        this(entityId, new ItemWrapper(itemStack));
    }

    public RecEntityOffHand(@NotNull ReplayByteBuffer reader) {
        this(
                new EntityId(reader),
