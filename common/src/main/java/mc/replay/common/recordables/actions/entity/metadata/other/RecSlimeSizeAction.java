...
public record RecSlimeSizeAction() implements EntityRecordableAction<RecSlimeSize> {

    @Override
    public @NotNull List<@NotNull ClientboundPacket> createPackets(@NotNull RecSlimeSize recordable, @UnknownNullability IEntityProvider provider) {
        ...
        if (entityMetadata instanceof SlimeMetadata slimeMetadata) {
            Metadata metadata = slimeMetadata.getMetadata();
