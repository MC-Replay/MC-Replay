...
public record RecZombieBecomingDrownedAction() implements EntityRecordableAction<RecZombieBecomingDrowned> {

    @Override
    public @NotNull List<@NotNull ClientboundPacket> createPackets(@NotNull RecZombieBecomingDrowned recordable, @UnknownNullability IEntityProvider provider) {
        ...
        if (entityMetadata instanceof ZombieMetadata zombieMetadata) {
            Metadata metadata = zombieMetadata.getMetadata();
