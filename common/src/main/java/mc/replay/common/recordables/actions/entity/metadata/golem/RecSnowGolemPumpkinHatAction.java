...
public record RecSnowGolemPumpkinHatAction() implements EntityRecordableAction<RecSnowGolemPumpkinHat> {

    @Override
    public @NotNull List<@NotNull ClientboundPacket> createPackets(@NotNull RecSnowGolemPumpkinHat recordable, @UnknownNullability IEntityProvider provider) {
        ...
        if (entityMetadata instanceof SnowGolemMetadata snowGolemMetadata) {
            Metadata metadata = snowGolemMetadata.getMetadata();
