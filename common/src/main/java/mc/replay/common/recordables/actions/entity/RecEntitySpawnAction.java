...
public record RecEntitySpawnAction() implements InternalEntityRecordableAction<RecEntitySpawn> {
    ...
    @Override
    public @NotNull List<@NotNull ClientboundPacket> createPackets(@NotNull RecEntitySpawn recordable, @UnknownNullability IReplayEntityProcessor data) {
        data.spawnEntity(recordable);
        return List.of();
    }
}